...
ManhattanPdiff <- read.table("BodyMasswithPDIFF.FDR.txt", header = TRUE)

#reformatted to create a manhattan plot only for the men's SNPs
names(ManhattanPdiff)[1] <- "rsid"
names(ManhattanPdiff)[2] <- "chrom"
names(ManhattanPdiff)[3] <- "pos"
ManhattanPdiff$chrom <- sub("^", "chr", ManhattanPdiff$chrom)

#designates colors for SNPs that are more significant in males vs females
for (i in 1:nrow(ManhattanPdiff)){
  if(ManhattanPdiff$WOMEN.pval[i]<ManhattanPdiff$MEN.pval[i]){
    ManhattanPdiff$color[i] = "#c94f7c"
  } else {ManhattanPdiff$color[i] = "#DDCC77"}
}

#Calculate FDR threshold for Body Mass (-log10(p-value) is different for each phenotype due to a different number of SNPs)
FirstFDR <- ManhattanPdiff %>% filter(pdiff.FDR<=0.05)
FirstFDR <- FirstFDR[order(-(FirstFDR$pdiff.FDR)),]
firstthreshold <- FirstFDR$pdiff[1]

SecondFDR <- ManhattanPdiff %>% filter(pdiff.FDR<=0.01)
SecondFDR <- SecondFDR[order(-(SecondFDR$pdiff.FDR)),]
secondthreshold <- SecondFDR$pdiff[1]

ThirdFDR <- ManhattanPdiff %>% filter(pdiff.FDR<=0.005)
ThirdFDR <- ThirdFDR[order(-(ThirdFDR$pdiff.FDR)),]
thirdthreshold <- ThirdFDR$pdiff[1]

FourthFDR <- ManhattanPdiff %>% filter(pdiff.FDR<=0.001)
FourthFDR <- FourthFDR[order(-(FourthFDR$pdiff.FDR)),]
fourththreshold <- FourthFDR$pdiff[1]

#FDR thresholds
first = -log10(firstthreshold) 
second = -log10(secondthreshold)
third = -log10(thirdthreshold) 
fourth = -log10(fourththreshold) 

ManhattanPdiff$y=-log10(ManhattanPdiff$pdiff)

ManhattanPdiff$pos <- as.numeric(as.character(ManhattanPdiff$pos))  

p <- manhattan(ManhattanPdiff, build='hg19') + 
  geom_hline(yintercept = fourth, size = 1.6, color = "#85c4c9") + 
  geom_hline(yintercept = third, size = 1.6, color = "#479aa2") + 
  geom_hline(yintercept = second, size = 1.6, color = "#37787e") +
  geom_hline(yintercept = first, size = 1.6, color = "#28565b") + 
  theme(axis.text.x = element_text(size=14), axis.text.y = element_text(size=14), axis.title=element_text(size=18))

ggsave(p, file="BodyMassManhattanPdiff.pdf", width=10, height=6, useDingbats=FALSE)

###################################################33
#my code to make a manhattan plot
GenomeWideSig <- -log10(5e-8)

ManhattanPdiff <- read.table("WaistCircwithPDIFF.FDR.txt", header = TRUE)
...
ggsave(p, file="WaistCircManhattanPdiff.pdf", width=10, height=6, useDingbats=FALSE)

###################################################33
#my code to make a manhattan plot
GenomeWideSig <- -log10(5e-8)

ManhattanPdiff <- read.table("HipCircwithPDIFF.FDR.txt", header = TRUE)
...
ggsave(p, file="HipCircManhattanPdiff.pdf", width=10, height=6, useDingbats=FALSE)

###################################################33
#my code to make a manhattan plot
GenomeWideSig <- -log10(5e-8)

ManhattanPdiff <- read.table("HeightwithPDIFF.FDR.txt", header = TRUE)
...
ggsave(p, file="HeightManhattanPdiff.pdf", width=10, height=6, useDingbats=FALSE)

###################################################33
#my code to make a manhattan plot
GenomeWideSig <- -log10(5e-8)

ManhattanPdiff <- read.table("BodyFatPercentwithPDIFF.FDR.txt", header = TRUE)
...
ggsave(p, file="BodyFatPercentManhattanPdiff.pdf", width=10, height=6, useDingbats=FALSE)