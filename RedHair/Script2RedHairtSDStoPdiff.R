...
Phenotype <- read.table("1747_2.gwas.FemalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
write.csv(Final.Phenotype.tSDS, "WomenRedHairwithtSDS.csv", row.names = FALSE)
...
write.csv(FinalBinSDSWOOD, "WomenRedHairSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("WomenRedHairSDSBins.csv")
...
ggsave("WomenRedHair.tSDSGraph.png")

#====================================================================
#--The following repeats the process for males
#====================================================================

Phenotype <- read.table("1747_2.gwas.MalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
#a complete table with every SNP and corresponding tSDS score
write.csv(Final.Phenotype.tSDS, "MenRedHairwithtSDS.csv", row.names = FALSE)
...
#writes a .csv file for use in the genome-wide plot
write.csv(FinalBinSDSWOOD, "MenRedHairSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("MenRedHairSDSBins.csv")

#makes a graph of the genome wide tSDS score of bins of 1000 SNPs, as seen in Field et al Figure 4A
ReplicatedFieldGraph %>% ggplot(aes(x=MeanP, y=MeantSDS)) + geom_point() + ggtitle("Phenotype tSDS score observed genome-wide")+ labs(x="Phenotype signifcance (rank)", y = "Phenotype-increasing tSDS (bin average)") + scale_x_reverse() + ylim(-.3,.75)

ggsave("MenRedHair.tSDSGraph.png")

WomenPheno <- read.csv("WomenRedHairwithtSDS.csv", header = TRUE)
MenPheno <- read.csv("MenRedHairwithtSDS.csv", header = TRUE)
...
RedHairPdiffvalues <- unname(unlist(Phenotype1Clean[,"pval"]))
fdr <- p.adjust(RedHairPdiffvalues, method = "fdr")
Phenotype1Clean$MEN.pval.FDR <- fdr

#calculates the FDR for women
RedHairPdiffvalues <- unname(unlist(Phenotype2Clean[,"pval"]))
fdr <- p.adjust(RedHairPdiffvalues, method = "fdr")
Phenotype2Clean$WOMEN.pval.FDR <- fdr

#total table with tSDS, FDR, and Bonferroni
write.table(Phenotype1Clean, "MenRedHair.tSDS.FDR.tsv", row.names = FALSE)
write.table(Phenotype2Clean, "WomenRedHair.tSDS.FDR.tsv", row.names = FALSE)
...
write.table(MenSmall, "RedHairMenManhattan.tsv", row.names = FALSE)
write.table(WomenSmall, "RedHairWomenManhattan.tsv", row.names = FALSE)

#---------------------------------------------------------------------
#this part of the script is used to get the Spearman correlation coefficient (rho)
#---------------------------------------------------------------------

RedHairMen <- read.table("MenRedHair.tSDS.FDR.tsv", header = TRUE)
RedHairWomen <- read.table ("WomenRedHair.tSDS.FDR.tsv", header = TRUE)

RedHairMen2 <- RedHairMen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, MEN.pval.FDR)
RedHairWomen2 <- RedHairWomen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, WOMEN.pval.FDR)

#changes the name of the column from Beta to differentiate for later
names(RedHairMen2)[8] <- "BETAM"
names(RedHairWomen2)[8] <- "BETAW"

#makes a table with only the marker name and beta values
BETARedHairMen <- RedHairMen2 %>% select(rsid, BETAM) #need to put in the MarkerName
BETARedHairWomen <- RedHairWomen2 %>% select(rsid, BETAW) #need to put in the MarkerName

#joins the tables so that there is a column of markernames with the beta values for
##men and women right next to each other
BETARedHair <- merge(BETARedHairMen, BETARedHairWomen, by = "rsid") 

#Spearman rank correlation test
RedHairCorr1 <- cor.test(~ BETAM + BETAW, data = BETARedHair, method = 'spearman', exact = FALSE)
RedHairCorr1
#grabs the rho value, needed later for the pdiff equation 
RedHairCorr1$estimate

#adds a column to both the men and women dataframes of the rho value
RedHairMen2=mutate(RedHairMen2, rho=RedHairCorr1$estimate)
RedHairWomen2=mutate(RedHairWomen2, rho=RedHairCorr1$estimate)

#makes a table for these that can be uploaded
write.table(RedHairMen2, "RedHairMenLessCol.tsv", row.names= FALSE, quote = FALSE)
write.table(RedHairWomen2, "RedHairWomenLessCol.tsv", row.names= FALSE, quote = FALSE)

#-----------------------------------------
#this part of the script is used to get the pdiff value
#-----------------------------------------

RedHairMen2 <- read.table("RedHairMenLessCol.tsv", header = TRUE)
RedHairWomen2 <- read.table("RedHairWomenLessCol.tsv", header = TRUE)
...
names(RedHairMen2)[7] <- "MEN.N"
names(RedHairMen2)[8] <- "MEN.beta"
names(RedHairMen2)[9] <- "MEN.se"
names(RedHairMen2)[10] <- "MEN.tstat"
names(RedHairMen2)[11] <- "MEN.pval"
names(RedHairMen2)[12] <- "MEN.tSDS"

names(RedHairWomen2)[7] <- "WOMEN.N"
names(RedHairWomen2)[8] <- "WOMEN.beta"
names(RedHairWomen2)[9] <- "WOMEN.se"
names(RedHairWomen2)[10] <- "WOMEN.tstat"
names(RedHairWomen2)[11] <- "WOMEN.pval"
names(RedHairWomen2)[12] <- "WOMEN.tSDS"

#concatenates the men and women values into the same datatable
UKBBRedHair <- merge(RedHairMen2, RedHairWomen2)

#cleaning data
UKBBRedHair$MEN.beta<-as.numeric(as.character(UKBBRedHair$MEN.beta)) #beta
UKBBRedHair$WOMEN.beta<-as.numeric(as.character(UKBBRedHair$WOMEN.beta)) #beta
UKBBRedHair$MEN.se <-as.numeric(as.character(UKBBRedHair$MEN.se)) #standarderror
UKBBRedHair$WOMEN.se <-as.numeric(as.character(UKBBRedHair$WOMEN.se)) #standarderror
UKBBRedHair$MEN.N<-as.numeric(as.character(UKBBRedHair$MEN.N)) #number of individuals
UKBBRedHair$WOMEN.N<-as.numeric(as.character(UKBBRedHair$WOMEN.N)) #number of individuals
UKBBRedHair$MEN.tSDS<-as.numeric(as.character(UKBBRedHair$MEN.tSDS)) #tSDS value
UKBBRedHair$WOMEN.tSDS<-as.numeric(as.character(UKBBRedHair$WOMEN.tSDS)) #tSDS value
UKBBRedHair$rho<-as.numeric(as.character(UKBBRedHair$rho)) #rho value from previous script
...
UKBBRedHair=mutate(UKBBRedHair,pdiff=pvalueF(UKBBRedHair$MEN.beta,UKBBRedHair$WOMEN.beta,UKBBRedHair$MEN.se,UKBBRedHair$WOMEN.se,UKBBRedHair$rho,(UKBBRedHair$MEN.N+UKBBRedHair$WOMEN.N)))

write.table(UKBBRedHair, "UKBBRedHairwithPDIFF.tsv", row.names = FALSE, quote = FALSE)
...
ConcatenatedSexRedHair <- read.table("UKBBRedHairwithPDIFF.tsv", header = TRUE)
...
WomenSig<- ConcatenatedSexRedHair %>% filter(WOMEN.pval <= cutoff)

#applies the cutoff to the men values and makes a new dataframe
MenSig<- ConcatenatedSexRedHair %>% filter(MEN.pval <= cutoff)

AllSig <- merge(MenSig, WomenSig, all=TRUE)

#calculates the FDR for the pdiff values
RedHairPdiffFDR <- unname(unlist(AllSig[,"pdiff"]))
fdr <- p.adjust(RedHairPdiffFDR, method = "fdr")
AllSig$pdiff.FDR <- fdr

write.table(AllSig, "RedHairwithPDIFF.FDR.tsv", row.names = FALSE)

#applies different FDR cutoffs for pdiff and writes a table
Ex1 <- AllSig %>% filter(pdiff.FDR <= .05)
write.table(Ex1, "RedHairPdiff1.tsv", row.names = FALSE)

Ex2 <- AllSig %>% filter(pdiff.FDR <= .01)
write.table(Ex2, "RedHairPdiff2.tsv", row.names = FALSE)

Ex3 <- AllSig %>% filter(pdiff.FDR <= .005)
write.table(Ex3, "RedHairPdiff3.tsv", row.names = FALSE)

Ex4 <- AllSig %>% filter(pdiff.FDR <= .001)
write.table(Ex4, "RedHairPdiff4.tsv", row.names = FALSE)
...
FDRtable <- read.table("RedHairPdiff4.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("RedHairPdiff3.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("RedHairPdiff2.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("RedHairPdiff1.tsv", header = TRUE)
...
write.table(tSDSTableMen, "RedHairtSDSTableMen.txt")
write.table(SNPsTableMen, "RedHairSNPsTableMen.txt")

write.table(tSDSTableWomen, "RedHairtSDSTableWomen.txt")
write.table(SNPsTableWomen, "RedHairSNPsTableWomen.txt")
