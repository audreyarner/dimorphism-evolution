...
Phenotype <- read.table("30130_irnt.gwas.FemalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
write.csv(Final.Phenotype.tSDS, "WomenMonocyteCountwithtSDS.csv", row.names = FALSE)
...
write.csv(FinalBinSDSWOOD, "WomenMonocyteCountSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("WomenMonocyteCountSDSBins.csv")
...
ggsave("WomenMonocyteCount.tSDSGraph.png")

#====================================================================
#--The following repeats the process for males
#====================================================================

Phenotype <- read.table("30130_irnt.gwas.MalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
#a complete table with every SNP and corresponding tSDS score
write.csv(Final.Phenotype.tSDS, "MenMonocyteCountwithtSDS.csv", row.names = FALSE)
...
#writes a .csv file for use in the genome-wide plot
write.csv(FinalBinSDSWOOD, "MenMonocyteCountSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("MenMonocyteCountSDSBins.csv")

#makes a graph of the genome wide tSDS score of bins of 1000 SNPs, as seen in Field et al Figure 4A
ReplicatedFieldGraph %>% ggplot(aes(x=MeanP, y=MeantSDS)) + geom_point() + ggtitle("Phenotype tSDS score observed genome-wide")+ labs(x="Phenotype signifcance (rank)", y = "Phenotype-increasing tSDS (bin average)") + scale_x_reverse() + ylim(-.3,.75)

ggsave("MenMonocyteCount.tSDSGraph.png")

WomenPheno <- read.csv("WomenMonocyteCountwithtSDS.csv", header = TRUE)
MenPheno <- read.csv("MenMonocyteCountwithtSDS.csv", header = TRUE)
...
MonocyteCountPdiffvalues <- unname(unlist(Phenotype1Clean[,"pval"]))
fdr <- p.adjust(MonocyteCountPdiffvalues, method = "fdr")
Phenotype1Clean$MEN.pval.FDR <- fdr

#calculates the FDR for women
MonocyteCountPdiffvalues <- unname(unlist(Phenotype2Clean[,"pval"]))
fdr <- p.adjust(MonocyteCountPdiffvalues, method = "fdr")
Phenotype2Clean$WOMEN.pval.FDR <- fdr

#total table with tSDS, FDR, and Bonferroni
write.table(Phenotype1Clean, "MenMonocyteCount.tSDS.FDR.tsv", row.names = FALSE)
write.table(Phenotype2Clean, "WomenMonocyteCount.tSDS.FDR.tsv", row.names = FALSE)
...
write.table(MenSmall, "MonocyteCountMenManhattan.tsv", row.names = FALSE)
write.table(WomenSmall, "MonocyteCountWomenManhattan.tsv", row.names = FALSE)

#---------------------------------------------------------------------
#this part of the script is used to get the Spearman correlation coefficient (rho)
#---------------------------------------------------------------------

MonocyteCountMen <- read.table("MenMonocyteCount.tSDS.FDR.tsv", header = TRUE)
MonocyteCountWomen <- read.table ("WomenMonocyteCount.tSDS.FDR.tsv", header = TRUE)

MonocyteCountMen2 <- MonocyteCountMen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, MEN.pval.FDR)
MonocyteCountWomen2 <- MonocyteCountWomen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, WOMEN.pval.FDR)

#changes the name of the column from Beta to differentiate for later
names(MonocyteCountMen2)[8] <- "BETAM"
names(MonocyteCountWomen2)[8] <- "BETAW"

#makes a table with only the marker name and beta values
BETAMonocyteCountMen <- MonocyteCountMen2 %>% select(rsid, BETAM) #need to put in the MarkerName
BETAMonocyteCountWomen <- MonocyteCountWomen2 %>% select(rsid, BETAW) #need to put in the MarkerName

#joins the tables so that there is a column of markernames with the beta values for
##men and women right next to each other
BETAMonocyteCount <- merge(BETAMonocyteCountMen, BETAMonocyteCountWomen, by = "rsid") 

#Spearman rank correlation test
MonocyteCountCorr1 <- cor.test(~ BETAM + BETAW, data = BETAMonocyteCount, method = 'spearman', exact = FALSE)
MonocyteCountCorr1
#grabs the rho value, needed later for the pdiff equation 
MonocyteCountCorr1$estimate

#adds a column to both the men and women dataframes of the rho value
MonocyteCountMen2=mutate(MonocyteCountMen2, rho=MonocyteCountCorr1$estimate)
MonocyteCountWomen2=mutate(MonocyteCountWomen2, rho=MonocyteCountCorr1$estimate)

#makes a table for these that can be uploaded
write.table(MonocyteCountMen2, "MonocyteCountMenLessCol.tsv", row.names= FALSE, quote = FALSE)
write.table(MonocyteCountWomen2, "MonocyteCountWomenLessCol.tsv", row.names= FALSE, quote = FALSE)

#-----------------------------------------
#this part of the script is used to get the pdiff value
#-----------------------------------------

MonocyteCountMen2 <- read.table("MonocyteCountMenLessCol.tsv", header = TRUE)
MonocyteCountWomen2 <- read.table("MonocyteCountWomenLessCol.tsv", header = TRUE)
...
names(MonocyteCountMen2)[7] <- "MEN.N"
names(MonocyteCountMen2)[8] <- "MEN.beta"
names(MonocyteCountMen2)[9] <- "MEN.se"
names(MonocyteCountMen2)[10] <- "MEN.tstat"
names(MonocyteCountMen2)[11] <- "MEN.pval"
names(MonocyteCountMen2)[12] <- "MEN.tSDS"

names(MonocyteCountWomen2)[7] <- "WOMEN.N"
names(MonocyteCountWomen2)[8] <- "WOMEN.beta"
names(MonocyteCountWomen2)[9] <- "WOMEN.se"
names(MonocyteCountWomen2)[10] <- "WOMEN.tstat"
names(MonocyteCountWomen2)[11] <- "WOMEN.pval"
names(MonocyteCountWomen2)[12] <- "WOMEN.tSDS"

#concatenates the men and women values into the same datatable
UKBBMonocyteCount <- merge(MonocyteCountMen2, MonocyteCountWomen2)

#cleaning data
UKBBMonocyteCount$MEN.beta<-as.numeric(as.character(UKBBMonocyteCount$MEN.beta)) #beta
UKBBMonocyteCount$WOMEN.beta<-as.numeric(as.character(UKBBMonocyteCount$WOMEN.beta)) #beta
UKBBMonocyteCount$MEN.se <-as.numeric(as.character(UKBBMonocyteCount$MEN.se)) #standarderror
UKBBMonocyteCount$WOMEN.se <-as.numeric(as.character(UKBBMonocyteCount$WOMEN.se)) #standarderror
UKBBMonocyteCount$MEN.N<-as.numeric(as.character(UKBBMonocyteCount$MEN.N)) #number of individuals
UKBBMonocyteCount$WOMEN.N<-as.numeric(as.character(UKBBMonocyteCount$WOMEN.N)) #number of individuals
UKBBMonocyteCount$MEN.tSDS<-as.numeric(as.character(UKBBMonocyteCount$MEN.tSDS)) #tSDS value
UKBBMonocyteCount$WOMEN.tSDS<-as.numeric(as.character(UKBBMonocyteCount$WOMEN.tSDS)) #tSDS value
UKBBMonocyteCount$rho<-as.numeric(as.character(UKBBMonocyteCount$rho)) #rho value from previous script
...
UKBBMonocyteCount=mutate(UKBBMonocyteCount,pdiff=pvalueF(UKBBMonocyteCount$MEN.beta,UKBBMonocyteCount$WOMEN.beta,UKBBMonocyteCount$MEN.se,UKBBMonocyteCount$WOMEN.se,UKBBMonocyteCount$rho,(UKBBMonocyteCount$MEN.N+UKBBMonocyteCount$WOMEN.N)))

write.table(UKBBMonocyteCount, "UKBBMonocyteCountwithPDIFF.tsv", row.names = FALSE, quote = FALSE)
...
ConcatenatedSexMonocyteCount <- read.table("UKBBMonocyteCountwithPDIFF.tsv", header = TRUE)
...
WomenSig<- ConcatenatedSexMonocyteCount %>% filter(WOMEN.pval <= cutoff)

#applies the cutoff to the men values and makes a new dataframe
MenSig<- ConcatenatedSexMonocyteCount %>% filter(MEN.pval <= cutoff)

AllSig <- merge(MenSig, WomenSig, all=TRUE)

#calculates the FDR for the pdiff values
MonocyteCountPdiffFDR <- unname(unlist(AllSig[,"pdiff"]))
fdr <- p.adjust(MonocyteCountPdiffFDR, method = "fdr")
AllSig$pdiff.FDR <- fdr

write.table(AllSig, "MonocyteCountwithPDIFF.FDR.tsv", row.names = FALSE)

#applies different FDR cutoffs for pdiff and writes a table
Ex1 <- AllSig %>% filter(pdiff.FDR <= .05)
write.table(Ex1, "MonocyteCountPdiff1.tsv", row.names = FALSE)

Ex2 <- AllSig %>% filter(pdiff.FDR <= .01)
write.table(Ex2, "MonocyteCountPdiff2.tsv", row.names = FALSE)

Ex3 <- AllSig %>% filter(pdiff.FDR <= .005)
write.table(Ex3, "MonocyteCountPdiff3.tsv", row.names = FALSE)

Ex4 <- AllSig %>% filter(pdiff.FDR <= .001)
write.table(Ex4, "MonocyteCountPdiff4.tsv", row.names = FALSE)
...
FDRtable <- read.table("MonocyteCountPdiff4.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("MonocyteCountPdiff3.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("MonocyteCountPdiff2.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("MonocyteCountPdiff1.tsv", header = TRUE)
...
write.table(tSDSTableMen, "MonocyteCounttSDSTableMen.txt")
write.table(SNPsTableMen, "MonocyteCountSNPsTableMen.txt")

write.table(tSDSTableWomen, "MonocyteCounttSDSTableWomen.txt")
write.table(SNPsTableWomen, "MonocyteCountSNPsTableWomen.txt")
