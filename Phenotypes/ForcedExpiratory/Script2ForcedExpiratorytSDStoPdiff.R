...
Phenotype <- read.table("20150_irnt.gwas.FemalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
write.csv(Final.Phenotype.tSDS, "WomenForcedExpiratorywithtSDS.csv", row.names = FALSE)
...
write.csv(FinalBinSDSWOOD, "WomenForcedExpiratorySDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("WomenForcedExpiratorySDSBins.csv")
...
ggsave("WomenForcedExpiratory.tSDSGraph.png")

#====================================================================
#--The following repeats the process for males
#====================================================================

Phenotype <- read.table("20150_irnt.gwas.MalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
#a complete table with every SNP and corresponding tSDS score
write.csv(Final.Phenotype.tSDS, "MenForcedExpiratorywithtSDS.csv", row.names = FALSE)
...
#writes a .csv file for use in the genome-wide plot
write.csv(FinalBinSDSWOOD, "MenForcedExpiratorySDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("MenForcedExpiratorySDSBins.csv")

#makes a graph of the genome wide tSDS score of bins of 1000 SNPs, as seen in Field et al Figure 4A
ReplicatedFieldGraph %>% ggplot(aes(x=MeanP, y=MeantSDS)) + geom_point() + ggtitle("Phenotype tSDS score observed genome-wide")+ labs(x="Phenotype signifcance (rank)", y = "Phenotype-increasing tSDS (bin average)") + scale_x_reverse() + ylim(-.3,.75)

ggsave("MenForcedExpiratory.tSDSGraph.png")

WomenPheno <- read.csv("WomenForcedExpiratorywithtSDS.csv", header = TRUE)
MenPheno <- read.csv("MenForcedExpiratorywithtSDS.csv", header = TRUE)
...
ForcedExpiratoryPdiffvalues <- unname(unlist(Phenotype1Clean[,"pval"]))
fdr <- p.adjust(ForcedExpiratoryPdiffvalues, method = "fdr")
Phenotype1Clean$MEN.pval.FDR <- fdr

#calculates the FDR for women
ForcedExpiratoryPdiffvalues <- unname(unlist(Phenotype2Clean[,"pval"]))
fdr <- p.adjust(ForcedExpiratoryPdiffvalues, method = "fdr")
Phenotype2Clean$WOMEN.pval.FDR <- fdr

#total table with tSDS, FDR, and Bonferroni
write.table(Phenotype1Clean, "MenForcedExpiratory.tSDS.FDR.tsv", row.names = FALSE)
write.table(Phenotype2Clean, "WomenForcedExpiratory.tSDS.FDR.tsv", row.names = FALSE)
...
write.table(MenSmall, "ForcedExpiratoryMenManhattan.tsv", row.names = FALSE)
write.table(WomenSmall, "ForcedExpiratoryWomenManhattan.tsv", row.names = FALSE)

#---------------------------------------------------------------------
#this part of the script is used to get the Spearman correlation coefficient (rho)
#---------------------------------------------------------------------

ForcedExpiratoryMen <- read.table("MenForcedExpiratory.tSDS.FDR.tsv", header = TRUE)
ForcedExpiratoryWomen <- read.table ("WomenForcedExpiratory.tSDS.FDR.tsv", header = TRUE)

ForcedExpiratoryMen2 <- ForcedExpiratoryMen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, MEN.pval.FDR)
ForcedExpiratoryWomen2 <- ForcedExpiratoryWomen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, WOMEN.pval.FDR)

#changes the name of the column from Beta to differentiate for later
names(ForcedExpiratoryMen2)[8] <- "BETAM"
names(ForcedExpiratoryWomen2)[8] <- "BETAW"

#makes a table with only the marker name and beta values
BETAForcedExpiratoryMen <- ForcedExpiratoryMen2 %>% select(rsid, BETAM) #need to put in the MarkerName
BETAForcedExpiratoryWomen <- ForcedExpiratoryWomen2 %>% select(rsid, BETAW) #need to put in the MarkerName

#joins the tables so that there is a column of markernames with the beta values for
##men and women right next to each other
BETAForcedExpiratory <- merge(BETAForcedExpiratoryMen, BETAForcedExpiratoryWomen, by = "rsid") 

#Spearman rank correlation test
ForcedExpiratoryCorr1 <- cor.test(~ BETAM + BETAW, data = BETAForcedExpiratory, method = 'spearman', exact = FALSE)
ForcedExpiratoryCorr1
#grabs the rho value, needed later for the pdiff equation 
ForcedExpiratoryCorr1$estimate

#adds a column to both the men and women dataframes of the rho value
ForcedExpiratoryMen2=mutate(ForcedExpiratoryMen2, rho=ForcedExpiratoryCorr1$estimate)
ForcedExpiratoryWomen2=mutate(ForcedExpiratoryWomen2, rho=ForcedExpiratoryCorr1$estimate)

#makes a table for these that can be uploaded
write.table(ForcedExpiratoryMen2, "ForcedExpiratoryMenLessCol.tsv", row.names= FALSE, quote = FALSE)
write.table(ForcedExpiratoryWomen2, "ForcedExpiratoryWomenLessCol.tsv", row.names= FALSE, quote = FALSE)

#-----------------------------------------
#this part of the script is used to get the pdiff value
#-----------------------------------------

ForcedExpiratoryMen2 <- read.table("ForcedExpiratoryMenLessCol.tsv", header = TRUE)
ForcedExpiratoryWomen2 <- read.table("ForcedExpiratoryWomenLessCol.tsv", header = TRUE)
...
names(ForcedExpiratoryMen2)[7] <- "MEN.N"
names(ForcedExpiratoryMen2)[8] <- "MEN.beta"
names(ForcedExpiratoryMen2)[9] <- "MEN.se"
names(ForcedExpiratoryMen2)[10] <- "MEN.tstat"
names(ForcedExpiratoryMen2)[11] <- "MEN.pval"
names(ForcedExpiratoryMen2)[12] <- "MEN.tSDS"

names(ForcedExpiratoryWomen2)[7] <- "WOMEN.N"
names(ForcedExpiratoryWomen2)[8] <- "WOMEN.beta"
names(ForcedExpiratoryWomen2)[9] <- "WOMEN.se"
names(ForcedExpiratoryWomen2)[10] <- "WOMEN.tstat"
names(ForcedExpiratoryWomen2)[11] <- "WOMEN.pval"
names(ForcedExpiratoryWomen2)[12] <- "WOMEN.tSDS"

#concatenates the men and women values into the same datatable
UKBBForcedExpiratory <- merge(ForcedExpiratoryMen2, ForcedExpiratoryWomen2)

#cleaning data
UKBBForcedExpiratory$MEN.beta<-as.numeric(as.character(UKBBForcedExpiratory$MEN.beta)) #beta
UKBBForcedExpiratory$WOMEN.beta<-as.numeric(as.character(UKBBForcedExpiratory$WOMEN.beta)) #beta
UKBBForcedExpiratory$MEN.se <-as.numeric(as.character(UKBBForcedExpiratory$MEN.se)) #standarderror
UKBBForcedExpiratory$WOMEN.se <-as.numeric(as.character(UKBBForcedExpiratory$WOMEN.se)) #standarderror
UKBBForcedExpiratory$MEN.N<-as.numeric(as.character(UKBBForcedExpiratory$MEN.N)) #number of individuals
UKBBForcedExpiratory$WOMEN.N<-as.numeric(as.character(UKBBForcedExpiratory$WOMEN.N)) #number of individuals
UKBBForcedExpiratory$MEN.tSDS<-as.numeric(as.character(UKBBForcedExpiratory$MEN.tSDS)) #tSDS value
UKBBForcedExpiratory$WOMEN.tSDS<-as.numeric(as.character(UKBBForcedExpiratory$WOMEN.tSDS)) #tSDS value
UKBBForcedExpiratory$rho<-as.numeric(as.character(UKBBForcedExpiratory$rho)) #rho value from previous script
...
UKBBForcedExpiratory=mutate(UKBBForcedExpiratory,pdiff=pvalueF(UKBBForcedExpiratory$MEN.beta,UKBBForcedExpiratory$WOMEN.beta,UKBBForcedExpiratory$MEN.se,UKBBForcedExpiratory$WOMEN.se,UKBBForcedExpiratory$rho,(UKBBForcedExpiratory$MEN.N+UKBBForcedExpiratory$WOMEN.N)))

write.table(UKBBForcedExpiratory, "UKBBForcedExpiratorywithPDIFF.tsv", row.names = FALSE, quote = FALSE)
...
ConcatenatedSexForcedExpiratory <- read.table("UKBBForcedExpiratorywithPDIFF.tsv", header = TRUE)
...
WomenSig<- ConcatenatedSexForcedExpiratory %>% filter(WOMEN.pval <= cutoff)

#applies the cutoff to the men values and makes a new dataframe
MenSig<- ConcatenatedSexForcedExpiratory %>% filter(MEN.pval <= cutoff)

AllSig <- merge(MenSig, WomenSig, all=TRUE)

#calculates the FDR for the pdiff values
ForcedExpiratoryPdiffFDR <- unname(unlist(AllSig[,"pdiff"]))
fdr <- p.adjust(ForcedExpiratoryPdiffFDR, method = "fdr")
AllSig$pdiff.FDR <- fdr

write.table(AllSig, "ForcedExpiratorywithPDIFF.FDR.tsv", row.names = FALSE)

#applies different FDR cutoffs for pdiff and writes a table
Ex1 <- AllSig %>% filter(pdiff.FDR <= .05)
write.table(Ex1, "ForcedExpiratoryPdiff1.tsv", row.names = FALSE)

Ex2 <- AllSig %>% filter(pdiff.FDR <= .01)
write.table(Ex2, "ForcedExpiratoryPdiff2.tsv", row.names = FALSE)

Ex3 <- AllSig %>% filter(pdiff.FDR <= .005)
write.table(Ex3, "ForcedExpiratoryPdiff3.tsv", row.names = FALSE)

Ex4 <- AllSig %>% filter(pdiff.FDR <= .001)
write.table(Ex4, "ForcedExpiratoryPdiff4.tsv", row.names = FALSE)
...
FDRtable <- read.table("ForcedExpiratoryPdiff4.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("ForcedExpiratoryPdiff3.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("ForcedExpiratoryPdiff2.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("ForcedExpiratoryPdiff1.tsv", header = TRUE)
...
write.table(tSDSTableMen, "ForcedExpiratorytSDSTableMen.txt")
write.table(SNPsTableMen, "ForcedExpiratorySNPsTableMen.txt")

write.table(tSDSTableWomen, "ForcedExpiratorytSDSTableWomen.txt")
write.table(SNPsTableWomen, "ForcedExpiratorySNPsTableWomen.txt")
