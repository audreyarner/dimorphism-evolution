...
write.csv(Final.Phenotype.tSDS, "WomenBladderCancerwithtSDS.csv", row.names = FALSE)
...
write.csv(FinalBinSDSWOOD, "WomenBladderCancerSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("WomenBladderCancerSDSBins.csv")
...
ggsave("WomenBladderCancer.tSDSGraph.png")
...
#a complete table with every SNP and corresponding tSDS score
write.csv(Final.Phenotype.tSDS, "MenBladderCancerwithtSDS.csv", row.names = FALSE)
...
#writes a .csv file for use in the genome-wide plot
write.csv(FinalBinSDSWOOD, "MenBladderCancerSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("MenBladderCancerSDSBins.csv")

#makes a graph of the genome wide tSDS score of bins of 1000 SNPs, as seen in Field et al Figure 4A
ReplicatedFieldGraph %>% ggplot(aes(x=MeanP, y=MeantSDS)) + geom_point() + ggtitle("Phenotype tSDS score observed genome-wide")+ labs(x="Phenotype signifcance (rank)", y = "Phenotype-increasing tSDS (bin average)") + scale_x_reverse() + ylim(-.3,.75)

ggsave("MenBladderCancer.tSDSGraph.png")

WomenPheno <- read.csv("WomenBladderCancerwithtSDS.csv", header = TRUE)
MenPheno <- read.csv("MenBladderCancerwithtSDS.csv", header = TRUE)
...
BladderCancerPdiffvalues <- unname(unlist(Phenotype1Clean[,"pval"]))
fdr <- p.adjust(BladderCancerPdiffvalues, method = "fdr")
Phenotype1Clean$MEN.pval.FDR <- fdr

#calculates the FDR for women
BladderCancerPdiffvalues <- unname(unlist(Phenotype2Clean[,"pval"]))
fdr <- p.adjust(BladderCancerPdiffvalues, method = "fdr")
Phenotype2Clean$WOMEN.pval.FDR <- fdr

#total table with tSDS, FDR, and Bonferroni
write.table(Phenotype1Clean, "MenBladderCancer.tSDS.FDR.tsv", row.names = FALSE)
write.table(Phenotype2Clean, "WomenBladderCancer.tSDS.FDR.tsv", row.names = FALSE)
...
write.table(MenSmall, "BladderCancerMenManhattan.tsv", row.names = FALSE)
write.table(WomenSmall, "BladderCancerWomenManhattan.tsv", row.names = FALSE)

#---------------------------------------------------------------------
#this part of the script is used to get the Spearman correlation coefficient (rho)
#---------------------------------------------------------------------

BladderCancerMen <- read.table("MenBladderCancer.tSDS.FDR.tsv", header = TRUE)
BladderCancerWomen <- read.table ("WomenBladderCancer.tSDS.FDR.tsv", header = TRUE)

BladderCancerMen2 <- BladderCancerMen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, MEN.pval.FDR)
BladderCancerWomen2 <- BladderCancerWomen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, WOMEN.pval.FDR)

#changes the name of the column from Beta to differentiate for later
names(BladderCancerMen2)[8] <- "BETAM"
names(BladderCancerWomen2)[8] <- "BETAW"

#makes a table with only the marker name and beta values
BETABladderCancerMen <- BladderCancerMen2 %>% select(rsid, BETAM) #need to put in the MarkerName
BETABladderCancerWomen <- BladderCancerWomen2 %>% select(rsid, BETAW) #need to put in the MarkerName

#joins the tables so that there is a column of markernames with the beta values for
##men and women right next to each other
BETABladderCancer <- merge(BETABladderCancerMen, BETABladderCancerWomen, by = "rsid") 

#Spearman rank correlation test
BladderCancerCorr1 <- cor.test(~ BETAM + BETAW, data = BETABladderCancer, method = 'spearman', exact = FALSE)
BladderCancerCorr1
#grabs the rho value, needed later for the pdiff equation 
BladderCancerCorr1$estimate

#adds a column to both the men and women dataframes of the rho value
BladderCancerMen2=mutate(BladderCancerMen2, rho=BladderCancerCorr1$estimate)
BladderCancerWomen2=mutate(BladderCancerWomen2, rho=BladderCancerCorr1$estimate)

#makes a table for these that can be uploaded
write.table(BladderCancerMen2, "BladderCancerMenLessCol.tsv", row.names= FALSE, quote = FALSE)
write.table(BladderCancerWomen2, "BladderCancerWomenLessCol.tsv", row.names= FALSE, quote = FALSE)

#-----------------------------------------
#this part of the script is used to get the pdiff value
#-----------------------------------------

BladderCancerMen2 <- read.table("BladderCancerMenLessCol.tsv", header = TRUE)
BladderCancerWomen2 <- read.table("BladderCancerWomenLessCol.tsv", header = TRUE)
...
names(BladderCancerMen2)[7] <- "MEN.N"
names(BladderCancerMen2)[8] <- "MEN.beta"
names(BladderCancerMen2)[9] <- "MEN.se"
names(BladderCancerMen2)[10] <- "MEN.tstat"
names(BladderCancerMen2)[11] <- "MEN.pval"
names(BladderCancerMen2)[12] <- "MEN.tSDS"

names(BladderCancerWomen2)[7] <- "WOMEN.N"
names(BladderCancerWomen2)[8] <- "WOMEN.beta"
names(BladderCancerWomen2)[9] <- "WOMEN.se"
names(BladderCancerWomen2)[10] <- "WOMEN.tstat"
names(BladderCancerWomen2)[11] <- "WOMEN.pval"
names(BladderCancerWomen2)[12] <- "WOMEN.tSDS"

#concatenates the men and women values into the same datatable
UKBBBladderCancer <- merge(BladderCancerMen2, BladderCancerWomen2)

#cleaning data
UKBBBladderCancer$MEN.beta<-as.numeric(as.character(UKBBBladderCancer$MEN.beta)) #beta
UKBBBladderCancer$WOMEN.beta<-as.numeric(as.character(UKBBBladderCancer$WOMEN.beta)) #beta
UKBBBladderCancer$MEN.se <-as.numeric(as.character(UKBBBladderCancer$MEN.se)) #standarderror
UKBBBladderCancer$WOMEN.se <-as.numeric(as.character(UKBBBladderCancer$WOMEN.se)) #standarderror
UKBBBladderCancer$MEN.N<-as.numeric(as.character(UKBBBladderCancer$MEN.N)) #number of individuals
UKBBBladderCancer$WOMEN.N<-as.numeric(as.character(UKBBBladderCancer$WOMEN.N)) #number of individuals
UKBBBladderCancer$MEN.tSDS<-as.numeric(as.character(UKBBBladderCancer$MEN.tSDS)) #tSDS value
UKBBBladderCancer$WOMEN.tSDS<-as.numeric(as.character(UKBBBladderCancer$WOMEN.tSDS)) #tSDS value
UKBBBladderCancer$rho<-as.numeric(as.character(UKBBBladderCancer$rho)) #rho value from previous script
...
UKBBBladderCancer=mutate(UKBBBladderCancer,pdiff=pvalueF(UKBBBladderCancer$MEN.beta,UKBBBladderCancer$WOMEN.beta,UKBBBladderCancer$MEN.se,UKBBBladderCancer$WOMEN.se,UKBBBladderCancer$rho,(UKBBBladderCancer$MEN.N+UKBBBladderCancer$WOMEN.N)))

write.table(UKBBBladderCancer, "UKBBBladderCancerwithPDIFF.tsv", row.names = FALSE, quote = FALSE)
...
ConcatenatedSexBladderCancer <- read.table("UKBBBladderCancerwithPDIFF.tsv", header = TRUE)
...
WomenSig<- ConcatenatedSexBladderCancer %>% filter(WOMEN.pval <= cutoff)

#applies the cutoff to the men values and makes a new dataframe
MenSig<- ConcatenatedSexBladderCancer %>% filter(MEN.pval <= cutoff)

AllSig <- merge(MenSig, WomenSig, all=TRUE)

#calculates the FDR for the pdiff values
BladderCancerPdiffFDR <- unname(unlist(AllSig[,"pdiff"]))
fdr <- p.adjust(BladderCancerPdiffFDR, method = "fdr")
AllSig$pdiff.FDR <- fdr

write.table(AllSig, "BladderCancerwithPDIFF.FDR.tsv", row.names = FALSE)

#applies different FDR cutoffs for pdiff and writes a table
Ex1 <- AllSig %>% filter(pdiff.FDR <= .05)
write.table(Ex1, "BladderCancerPdiff1.tsv", row.names = FALSE)

Ex2 <- AllSig %>% filter(pdiff.FDR <= .01)
write.table(Ex2, "BladderCancerPdiff2.tsv", row.names = FALSE)

Ex3 <- AllSig %>% filter(pdiff.FDR <= .005)
write.table(Ex3, "BladderCancerPdiff3.tsv", row.names = FALSE)

Ex4 <- AllSig %>% filter(pdiff.FDR <= .001)
write.table(Ex4, "BladderCancerPdiff4.tsv", row.names = FALSE)
...
FDRtable <- read.table("BladderCancerPdiff4.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("BladderCancerPdiff3.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("BladderCancerPdiff2.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("BladderCancerPdiff1.tsv", header = TRUE)
...
write.table(tSDSTableMen, "BladderCancertSDSTableMen.txt")
write.table(SNPsTableMen, "BladderCancerSNPsTableMen.txt")

write.table(tSDSTableWomen, "BladderCancertSDSTableWomen.txt")
write.table(SNPsTableWomen, "BladderCancerSNPsTableWomen.txt")
