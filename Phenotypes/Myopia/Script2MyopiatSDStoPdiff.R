...
Phenotype <- read.table("5843_3.gwas.FemalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
write.csv(Final.Phenotype.tSDS, "WomenMyopiawithtSDS.csv", row.names = FALSE)
...
write.csv(FinalBinSDSWOOD, "WomenMyopiaSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("WomenMyopiaSDSBins.csv")
...
ggsave("WomenMyopia.tSDSGraph.png")

#====================================================================
#--The following repeats the process for males
#====================================================================

Phenotype <- read.table("5843_3.gwas.MalewithRSID.tsv", header = TRUE)
nrow(Phenotype)
...
#a complete table with every SNP and corresponding tSDS score
write.csv(Final.Phenotype.tSDS, "MenMyopiawithtSDS.csv", row.names = FALSE)
...
#writes a .csv file for use in the genome-wide plot
write.csv(FinalBinSDSWOOD, "MenMyopiaSDSBins.csv", row.names = FALSE)

ReplicatedFieldGraph <- read.csv("MenMyopiaSDSBins.csv")

#makes a graph of the genome wide tSDS score of bins of 1000 SNPs, as seen in Field et al Figure 4A
ReplicatedFieldGraph %>% ggplot(aes(x=MeanP, y=MeantSDS)) + geom_point() + ggtitle("Phenotype tSDS score observed genome-wide")+ labs(x="Phenotype signifcance (rank)", y = "Phenotype-increasing tSDS (bin average)") + scale_x_reverse() + ylim(-.3,.75)

ggsave("MenMyopia.tSDSGraph.png")

WomenPheno <- read.csv("WomenMyopiawithtSDS.csv", header = TRUE)
MenPheno <- read.csv("MenMyopiawithtSDS.csv", header = TRUE)
...
MyopiaPdiffvalues <- unname(unlist(Phenotype1Clean[,"pval"]))
fdr <- p.adjust(MyopiaPdiffvalues, method = "fdr")
Phenotype1Clean$MEN.pval.FDR <- fdr

#calculates the FDR for women
MyopiaPdiffvalues <- unname(unlist(Phenotype2Clean[,"pval"]))
fdr <- p.adjust(MyopiaPdiffvalues, method = "fdr")
Phenotype2Clean$WOMEN.pval.FDR <- fdr

#total table with tSDS, FDR, and Bonferroni
write.table(Phenotype1Clean, "MenMyopia.tSDS.FDR.tsv", row.names = FALSE)
write.table(Phenotype2Clean, "WomenMyopia.tSDS.FDR.tsv", row.names = FALSE)
...
write.table(MenSmall, "MyopiaMenManhattan.tsv", row.names = FALSE)
write.table(WomenSmall, "MyopiaWomenManhattan.tsv", row.names = FALSE)

#---------------------------------------------------------------------
#this part of the script is used to get the Spearman correlation coefficient (rho)
#---------------------------------------------------------------------

MyopiaMen <- read.table("MenMyopia.tSDS.FDR.tsv", header = TRUE)
MyopiaWomen <- read.table ("WomenMyopia.tSDS.FDR.tsv", header = TRUE)

MyopiaMen2 <- MyopiaMen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, MEN.pval.FDR)
MyopiaWomen2 <- MyopiaWomen %>% select(rsid, CHR, POS, AA, DA, variant, n_complete_samples, beta, se, tstat, pval, tSDS, WOMEN.pval.FDR)

#changes the name of the column from Beta to differentiate for later
names(MyopiaMen2)[8] <- "BETAM"
names(MyopiaWomen2)[8] <- "BETAW"

#makes a table with only the marker name and beta values
BETAMyopiaMen <- MyopiaMen2 %>% select(rsid, BETAM) #need to put in the MarkerName
BETAMyopiaWomen <- MyopiaWomen2 %>% select(rsid, BETAW) #need to put in the MarkerName

#joins the tables so that there is a column of markernames with the beta values for
##men and women right next to each other
BETAMyopia <- merge(BETAMyopiaMen, BETAMyopiaWomen, by = "rsid") 

#Spearman rank correlation test
MyopiaCorr1 <- cor.test(~ BETAM + BETAW, data = BETAMyopia, method = 'spearman', exact = FALSE)
MyopiaCorr1
#grabs the rho value, needed later for the pdiff equation 
MyopiaCorr1$estimate

#adds a column to both the men and women dataframes of the rho value
MyopiaMen2=mutate(MyopiaMen2, rho=MyopiaCorr1$estimate)
MyopiaWomen2=mutate(MyopiaWomen2, rho=MyopiaCorr1$estimate)

#makes a table for these that can be uploaded
write.table(MyopiaMen2, "MyopiaMenLessCol.tsv", row.names= FALSE, quote = FALSE)
write.table(MyopiaWomen2, "MyopiaWomenLessCol.tsv", row.names= FALSE, quote = FALSE)

#-----------------------------------------
#this part of the script is used to get the pdiff value
#-----------------------------------------

MyopiaMen2 <- read.table("MyopiaMenLessCol.tsv", header = TRUE)
MyopiaWomen2 <- read.table("MyopiaWomenLessCol.tsv", header = TRUE)
...
names(MyopiaMen2)[7] <- "MEN.N"
names(MyopiaMen2)[8] <- "MEN.beta"
names(MyopiaMen2)[9] <- "MEN.se"
names(MyopiaMen2)[10] <- "MEN.tstat"
names(MyopiaMen2)[11] <- "MEN.pval"
names(MyopiaMen2)[12] <- "MEN.tSDS"

names(MyopiaWomen2)[7] <- "WOMEN.N"
names(MyopiaWomen2)[8] <- "WOMEN.beta"
names(MyopiaWomen2)[9] <- "WOMEN.se"
names(MyopiaWomen2)[10] <- "WOMEN.tstat"
names(MyopiaWomen2)[11] <- "WOMEN.pval"
names(MyopiaWomen2)[12] <- "WOMEN.tSDS"

#concatenates the men and women values into the same datatable
UKBBMyopia <- merge(MyopiaMen2, MyopiaWomen2)

#cleaning data
UKBBMyopia$MEN.beta<-as.numeric(as.character(UKBBMyopia$MEN.beta)) #beta
UKBBMyopia$WOMEN.beta<-as.numeric(as.character(UKBBMyopia$WOMEN.beta)) #beta
UKBBMyopia$MEN.se <-as.numeric(as.character(UKBBMyopia$MEN.se)) #standarderror
UKBBMyopia$WOMEN.se <-as.numeric(as.character(UKBBMyopia$WOMEN.se)) #standarderror
UKBBMyopia$MEN.N<-as.numeric(as.character(UKBBMyopia$MEN.N)) #number of individuals
UKBBMyopia$WOMEN.N<-as.numeric(as.character(UKBBMyopia$WOMEN.N)) #number of individuals
UKBBMyopia$MEN.tSDS<-as.numeric(as.character(UKBBMyopia$MEN.tSDS)) #tSDS value
UKBBMyopia$WOMEN.tSDS<-as.numeric(as.character(UKBBMyopia$WOMEN.tSDS)) #tSDS value
UKBBMyopia$rho<-as.numeric(as.character(UKBBMyopia$rho)) #rho value from previous script
...
UKBBMyopia=mutate(UKBBMyopia,pdiff=pvalueF(UKBBMyopia$MEN.beta,UKBBMyopia$WOMEN.beta,UKBBMyopia$MEN.se,UKBBMyopia$WOMEN.se,UKBBMyopia$rho,(UKBBMyopia$MEN.N+UKBBMyopia$WOMEN.N)))

write.table(UKBBMyopia, "UKBBMyopiawithPDIFF.tsv", row.names = FALSE, quote = FALSE)
...
ConcatenatedSexMyopia <- read.table("UKBBMyopiawithPDIFF.tsv", header = TRUE)
...
WomenSig<- ConcatenatedSexMyopia %>% filter(WOMEN.pval <= cutoff)

#applies the cutoff to the men values and makes a new dataframe
MenSig<- ConcatenatedSexMyopia %>% filter(MEN.pval <= cutoff)

AllSig <- merge(MenSig, WomenSig, all=TRUE)

#calculates the FDR for the pdiff values
MyopiaPdiffFDR <- unname(unlist(AllSig[,"pdiff"]))
fdr <- p.adjust(MyopiaPdiffFDR, method = "fdr")
AllSig$pdiff.FDR <- fdr

write.table(AllSig, "MyopiawithPDIFF.FDR.tsv", row.names = FALSE)

#applies different FDR cutoffs for pdiff and writes a table
Ex1 <- AllSig %>% filter(pdiff.FDR <= .05)
write.table(Ex1, "MyopiaPdiff1.tsv", row.names = FALSE)

Ex2 <- AllSig %>% filter(pdiff.FDR <= .01)
write.table(Ex2, "MyopiaPdiff2.tsv", row.names = FALSE)

Ex3 <- AllSig %>% filter(pdiff.FDR <= .005)
write.table(Ex3, "MyopiaPdiff3.tsv", row.names = FALSE)

Ex4 <- AllSig %>% filter(pdiff.FDR <= .001)
write.table(Ex4, "MyopiaPdiff4.tsv", row.names = FALSE)
...
FDRtable <- read.table("MyopiaPdiff4.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("MyopiaPdiff3.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("MyopiaPdiff2.tsv", header = TRUE)
...
##########################
FDRtable <- read.table("MyopiaPdiff1.tsv", header = TRUE)
...
write.table(tSDSTableMen, "MyopiatSDSTableMen.txt")
write.table(SNPsTableMen, "MyopiaSNPsTableMen.txt")

write.table(tSDSTableWomen, "MyopiatSDSTableWomen.txt")
write.table(SNPsTableWomen, "MyopiaSNPsTableWomen.txt")
